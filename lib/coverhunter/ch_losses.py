...
class FocalLoss(nn.Module):
    ...
    def forward_with_onehot(
        ...
        weight = torch.mul(y_true, torch.pow(torch.sub(1.0, y_pred), self._gamma))
...
class HardTripletLoss(nn.Module):
    ...
    def forward(self, embeddings, labels, ids=None):
        """
        Args:
    ...
    @staticmethod
    def _get_anchor_positive_triplet_mask(labels, ids):
        ...
        if ids is None:
            ...
            indices_not_equal = (
                torch.unsqueeze(ids, 0) == torch.unsqueeze(ids, 1)
            ).byte() ^ 1
